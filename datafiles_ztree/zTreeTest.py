    ...
    for file in os.listdir(directory):
        if file.endswith(".txt"):
            ...
            for i in range (len (lines) -  1):
	    		...
	    		if (i > 0 and sends[i-1] != -1):
	    			trusts.append (float (line.split()[1].replace ("\"","")))
	    		sends.append (float (line.split()[2].replace ("\"","")))
            ...
            for i in range (len (trusts)):
                differences.append (abs (py_trusts[i] - trusts[i]) > 0.01)
                if (abs (py_trusts[i] - trusts[i]) > 0.01):
                    print "-----"
                    print "Something wrong. Please check"
                    print file
                    print "{0:.2f}".format(sends[i]) + "\t" + str(py_trusts[i]) + "\t" + "{0:.2f}".format(trusts[i]) + "\t" + "{0:.2f}".format(py_trusts[i] - trusts[i])
					
    print "The average difference of trust score calculation between zTree and Python is: " + "{0:.3f}".format (sum(differences)/len(differences))

# import sys
# if len (sys.argv) >= 2:
#     testZTreeData (sys.argv[1])
# else:
#     print "Missing argument. Please provide the directory contains TXT file (data from zTree)"
