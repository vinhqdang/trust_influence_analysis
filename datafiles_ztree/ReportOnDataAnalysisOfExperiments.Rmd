  ...
  for (k in 1:num_games) {
    ...
    if (type == 1) {
      ...
      if (k == 4) {
        ...
        # Remove two cases in Receiver Combine game where they sent back 1.0
        print ("Predict without subject ID 2 and 6 because they sent back outlier (1.0)")
        print (summary (lm (RelSend ~ my_trust_value + trust_value + AbsPartnerSend, data = df_round5[df_round5$id != 2 & df_round5$id != 6,])))
        print (summary (lm (RelSend ~ my_reputation + partner_reputation + AbsPartnerSend, data = df_round5[df_round5$id != 2 & df_round5$id != 6,])))
      }
...
## Compare the variance between game

Date: Mon 26-Jun-2017

When we divided the data to high variance and low variance users, only Receiver - Score Game is strange. In other cases, trust is a better predictive model in high variance section while reputation is a better one in low variance part.

In this analysis, I use F-test to compare variance between game.
...
for (type in 0:1) {
  ...
  for (k in 1:num_games) {
    ...
    all_vars <- c(all_vars, as.vector(vars$x))
  }
  
  # compare variance by F-test
  # create groups id
  require(PairedData)
  groups <- c(rep(1,30), rep(2,30), rep(3,30), rep(4,30))
  groups <- as.factor(groups)
  for (k in 1:num_games) {
    for (j in (k+1):num_games) {
      if (k < j & j <= 4) {
        # print (paste (k, "----", j))
        print (paste("Compare variance between", GAME_NAMES[k], "and", GAME_NAMES[j]))
        x1 <- all_vars[((k-1)*30 + 1):(k*30)]
        x2 <- all_vars[((j-1)*30 + 1):(j*30)]
        # print (var.test(all_vars[((k-1)*30 + 1):(k*30)], all_vars[((j-1)*30 + 1):(j*30)], paired = FALSE))
        # print (PairedData::var.test.paired(x1, x2))
        print (var.test(x1,x2, paired=TRUE))
        print ("Fligner test")
        print (fligner.test(c(x1,x2) ~ c(rep(k,30), rep (j,30))))
      }
    }
  }
}
...
##Predict send proportion based on CBDT
...
# Date: 9-Aug-2017
# Study the change of CBDT predictive power
type <- 0
k <- 1  # Game ID (1..4)

SHOW_TRUST = ifelse((k == 1 | k == 2),0,1)
SHOW_ID = ifelse((k==1 | k == 3), 0, 1)

p_recalls <- seq (0.2,1,0.05)

results <- data.frame(r_cbdt = as.numeric(),
                      r_trust = as.numeric(),
                      rmse_cbdt = as.numeric(),
                      rmse_trust = as.numeric()
                      )

for (p in p_recalls) {
  res <- CBDT_predict(cur_df = as.data.frame(all_df_sender[k]), p_recall = p, type = type, 
                      show_trust = SHOW_TRUST, show_id = SHOW_ID
                      )
  results [nrow(results) + 1,] <- res
}

plot (results$r_cbdt ~ p_recalls, type = "o", ylim = c(0,1), col = col_list[1])
lines (results$rmse_cbdt ~ p_recalls, type = "o", col = col_list[2])

legend(x=0.8, y = 1, col = col_list[1:2], lty =c(1,1), c("R2", "RMSE"))
```

Now we perform CBDT prediction for all types (sender / receiver) in all games

```{r, echo=FALSE}
for (type in 0:1) {
  for (k in 1:4) {
    print (paste ("CBDT prediction for:", Type_names[type+1], " in game:", GAME_NAMES[k]))
    SHOW_TRUST = ifelse((k == 1 | k == 2),0,1)
    SHOW_ID = ifelse((k==1 | k == 3), 0, 1)
    res <- CBDT_predict(cur_df = as.data.frame(all_df_sender[k]), p_recall = .5, type = type, 
                      show_trust = SHOW_TRUST, show_id = SHOW_ID,
                      round_number = 5)
    print ("   ----    ")
