...
```{r, echo = FALSE}
  for (type in 0:1) {
    ...
    g1 <- glm(send0 ~ id + show_trust+ show_id,family=binomial(link='logit'),data=df0)
    print (summary (g1))
...
```{r, echo = FALSE}
  for (type in 0:1) {
    ...
    for (k in 1:num_games) {
      ...
      cur_df <- cur_df [cur_df$round_number_with_partner != 1,]
      ...
      if (type == 0) {
        # l1 <- lm (RelSend ~ id + my_trust_value + trust_value, data = cur_game)
        l1 <- lm (resid ~ my_trust_value + my_reputation + trust_value + partner_reputation, data=cur_df)
        print (summary (l1))
        # print (Anova(l1, type = "III"))
        ...
        # l1 <- lm (RelSend ~ id + my_trust_value + trust_value, data = cur_game)
        l1 <- lm (resid ~ my_trust_value + my_reputation + trust_value + partner_reputation + AbsPartnerSend, data = cur_df)
        print (summary (l1))
        # print (Anova(l1, type = "III"))
